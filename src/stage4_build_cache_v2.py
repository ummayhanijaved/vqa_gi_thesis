...
# ─────────────────────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────────────────────
# YOLO inference (routes 4-5) — CORRECT: joins by img_id from cache record.
# No infer_route, no HF iteration, no positional guessing. Each cache record
# already knows its own img_id and its own (stored) route.
# ─────────────────────────────────────────────────────────────────────────────
def run_yolo_route_with_images(records, yolo_seg, yolo_det,
                               hf_dataset, split_name):
    """
    records: the WITH-IMGID stage3 cache. Each record has 'route' and 'img_id'.
    hf_dataset is no longer needed for routing — kept in signature for
    ...
    # Indices of route-4 and route-5 records (by STORED route — the one
    # Stage 4 was trained on). img_id comes straight from each record.
    route4 = [(i, records[i]["img_id"]) for i in range(len(records))
              if records[i].get("route") == 4]
    route5 = [(i, records[i]["img_id"]) for i in range(len(records))
              if records[i].get("route") == 5]
    print(f"\n  Route 4 (location): {len(route4):,} records")
    print(f"  Route 5 (count):    {len(route5):,} records")

    # ── Route 4: YOLO-Seg, with checkpoint/resume ─────────────────────────
    if yolo_seg is not None and route4:
        partial_path = os.path.join(CFG["stage4_cache_dir"],
                                    f"partial_r4_{split_name}.pt")
        if os.path.exists(partial_path):
            predictions.update(torch.load(partial_path, map_location="cpu",
                                          weights_only=False))
        from collections import Counter
        pc = Counter()
        for n, (cache_idx, img_id) in enumerate(
                tqdm(route4, desc="   YOLO-Seg")):
            if cache_idx in predictions:
                continue
            ...
            if img_path is None:
                ...
                try:
                    res = yolo_seg(img_path, verbose=False)[0]
            ...
            if (n + 1) % CFG["CHECKPOINT_EVERY"] == 0:
                torch.save({k: v for k, v in predictions.items()
                            if any(k == ci for ci, _ in route4)}, partial_path)
            if (n + 1) % CFG["GC_EVERY"] == 0:
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
        print(f"   Top 5 Route 4 predictions:")
        for p, c in pc.most_common(5):
            print(f"     '{p[:40]:<40}' {c:>6}")
        if os.path.exists(partial_path):
            os.remove(partial_path)

    # ── Route 5: YOLO-Det, with checkpoint/resume ─────────────────────────
    if yolo_det is not None and route5:
        partial_path = os.path.join(CFG["stage4_cache_dir"],
        ...
        for n, (cache_idx, img_id) in enumerate(
            ...
            if img_path is None:
                ...
                try:
                    res = yolo_det(img_path, verbose=False)[0]
            ...
            if (n + 1) % CFG["CHECKPOINT_EVERY"] == 0:
                torch.save({k: v for k, v in predictions.items()
                            if any(k == ci for ci, _ in route5)}, partial_path)
            if (n + 1) % CFG["GC_EVERY"] == 0:
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
...
def build_cache_for_split(split, distilbert_models, tokenizer,
    ...
    s3_path = os.path.join(CFG["stage3_cache_dir"], f"stage3_cache_{split}_with_imgid.pt")
