...
for the revised DistilBERT + YOLO architecture:
    ...
    8.  Vocab Coverage Analysis    (GT-token mismatch per route)
...
NOTES:
   - Figures use route-specific scores, not one uniform accuracy metric.
   - Example figure reports question availability honestly; saved eval CSVs may
     not contain the original question text.
...
def plot_test_performance(eval_csvs):
    ...
    ax1.set_ylabel("Route-specific test score (%)", fontsize=12)
    ax1.set_title("Phase 2 Per-Route Test Performance",
    ...
    ax2.invert_yaxis()  # lowest score appears at the top = hardest first
    for i, a in enumerate(raccs):
        ax2.text(a + 1, i, f"{a:.2f}%", va="center", fontsize=10)
    ax2.set_xlabel("Route-specific test score (%)")
...
def plot_full_pipeline_summary():
    ...
    ax.set_ylabel("Stage-specific performance score (%)", fontsize=13)
    ax.set_title("Full VQA Pipeline Performance Summary",
                 fontsize=14, fontweight="bold")
    ax.set_ylim(0, 105)
...
def plot_error_analysis(eval_csvs):
    ...
    fig.suptitle("Phase 2 Strict Error Analysis — Common Misprediction Patterns",
...
def plot_confidence_analysis(eval_csvs):
    ...
    fig.suptitle("Phase 2 Strict Correct/Wrong Distribution with Sample Counts",
                 fontweight="bold", fontsize=14)
    ...
    for ax, route in zip(axes.flat, range(6)):
        ...
        ax.set_title(f"Route {route} — {ROUTE_LABELS[route]}\n"
                     f"Strict score: {acc:.2f}% ({n_tot} samples)",
                     fontweight="bold", fontsize=10)
...
def plot_disease_influence(eval_csvs, test_records=None):
    ...
    ax.set_title("Per-Disease Strict Correctness — All 6 Routes × 23 Diseases",
                 fontweight="bold")
    plt.colorbar(im, ax=ax, fraction=0.04, pad=0.02, label="Strict correctness (%)")
    plt.tight_layout()
    out = os.path.join(OUT_DIR, "07_disease_influence.png")
    plt.savefig(out, dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_vocab_coverage(eval_csvs):
    ...
    axes[1].set_ylabel("GT-token mismatch rate (%)")
    axes[1].set_title("Ground-Truth Tokens Missing from Predictions",
                      fontweight="bold")
...
def plot_end_to_end_examples(eval_csvs):
    ...
    fig.suptitle("Phase 2 Prediction Examples — All 6 Routes",
                 fontweight="bold", fontsize=14)
...
# ─────────────────────────────────────────────────────────────────────────────
def plot_latency(hardware="NVIDIA RTX 5060 Laptop GPU"):
...
def plot_stage_comparison():
    ...
    cols = ["Stage", "Component", "Architecture", "Main Score"]
...
def main():
    ...
    parser.add_argument("--hardware", default=os.environ.get("STAGE4_HARDWARE", "NVIDIA RTX 5060 Laptop GPU"),
                         help="Hardware label shown in latency figure title")
