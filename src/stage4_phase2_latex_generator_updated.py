...
NOTES:
  - Generated captions use route-specific score wording because Stage 4
    routes use different evaluation criteria.
  - Saved eval CSVs may not contain the original question text; examples are
    labelled honestly in that case.
...
def latex_escape(s, max_len=None):
    """Escape text for safe insertion into LaTeX tables."""
    s = str(s)
...
\begin{axis}[
    ...
    ylabel={Route-specific test score (\%)}, xlabel={Question Category},
    ...
    title={\textbf{Phase 2 Per-Route Test Performance}},
]
...
\end{axis}
\end{tikzpicture}
\caption{Phase 2 per-route test performance on Kvasir-VQA-x1. Scores are route-specific and should not be interpreted as one uniform accuracy metric.}
...
\begin{axis}[
    ...
    ymin=0, ymax=105,
    ylabel={Stage-specific performance score (\%)}, xlabel={Pipeline Stage},
    ...
    title={\textbf{Full VQA Pipeline Performance Summary}},
]
...
\end{axis}
\end{tikzpicture}
\caption{Stage-wise performance summary of the full VQA pipeline. The reported values represent the main score used at each stage and are not directly comparable as a single uniform accuracy metric.}
...
def gen_error_analysis(eval_csvs):
    ...
    tex = r"""\begin{table}[H]
\centering
\caption{Phase 2 top-3 most frequent strict mispredictions per route.}
...
\begin{axis}[
    ...
    grid=major, grid style={dashed,gray!30},
    title={\textbf{Phase 2 Strict Correct vs Wrong Sample Counts}},
]
...
\end{axis}
\end{tikzpicture}
\caption{Phase 2 absolute correct/wrong sample counts per route using strict matching. Spatial routes may score higher under route-specific fuzzy or tolerance-based evaluation.}
...
# ─────────────────────────────────────────────────────────────────────────────
# 8. VOCAB COVERAGE — vocab size per route + GT-token mismatch rate
# ─────────────────────────────────────────────────────────────────────────────
...
\begin{axis}[
    width=\textwidth, height=5.5cm, ybar, bar width=14pt,
    ymin=0, ymax=100, ylabel={GT-token mismatch rate (\%)},
    symbolic x coords={R0, R1, R2, R3, R4, R5}, xtick=data,
    nodes near coords, nodes near coords style={font=\footnotesize},
    grid=major, grid style={dashed,gray!30},
    title={\textbf{Ground-truth tokens missing from predictions}},
]
...
(log scale). Right: GT-token mismatch rate -- percentage of ground-truth tokens that never appear in any prediction. This is a token-overlap mismatch measure, not tokenizer OOV.}
\label{fig:p2_vocab}
\end{figure}
"""
...
def gen_examples(eval_csvs):
    ...
    table_rows = []
    has_questions = ("question" in df.columns and
                     df["question"].astype(str).str.strip().ne("").any())
    for _, row in df.iterrows():
        ok = r"\checkmark" if row["correct"] else r"\ding{55}"
        question = row["question"] if has_questions and str(row["question"]).strip() else \
                   "Question not stored in eval CSV"
        table_rows.append(
            f"R{row['route']} & {latex_escape(question, 60)} & "
            f"{latex_escape(row['gt'], 30)} & "
            f"{latex_escape(row['prediction'], 30)} & {ok} \\")

    tex = r"""\begin{table}[H]
\centering
\small
\caption{Phase 2 sample predictions across all six routes. If the saved evaluation CSV does not contain the original question field, this table reports that honestly instead of leaving the question blank.}
...
# ─────────────────────────────────────────────────────────────────────────────
def gen_latency(hardware="NVIDIA RTX 5060 Laptop GPU"):
    print("[10/12] Latency ...")
...
\begin{axis}[
    ...
    grid=major, grid style={dashed,gray!30},
    legend pos=north west, legend cell align=left,
    title={\textbf{Phase 2 Per-Route Inference Latency}},
]
\addplot[fill=blue!60] coordinates {
    """ + coords_mean + r"""
};
...
\end{axis}
\end{tikzpicture}
""" + f"\\caption{{Phase 2 single-sample inference latency per route on {latex_escape(hardware)}. DistilBERT-based routes are lightweight, while YOLO-based routes require more computation for detection or segmentation.}}\n" + r"""\label{fig:p2_latency}
\end{figure}
"""
...
def gen_stage_comparison():
    ...
    tex = r"""\begin{table}[H]
\centering
\caption{Full VQA pipeline stage-by-stage summary. Scores are stage-specific and should not be read as one uniform accuracy metric.}
\label{tab:full_pipeline}
\begin{tabular}{llll}
\toprule
\textbf{Stage} & \textbf{Component} & \textbf{Architecture} & \textbf{Main Score} \\
\midrule
""" + rows + r"""
\bottomrule
\end{tabular}
...
\end{axis}
\end{tikzpicture}
\caption{Phase 2 mean answer length per route. The route-specialised predictors produce short structured answers, while ground-truth answers are often longer clinical descriptions. This length mismatch motivates Stage~5 verbalisation.}
...
def main():
    ...
    parser.add_argument("--hardware", default=os.environ.get(
        "STAGE4_HARDWARE", "NVIDIA RTX 5060 Laptop GPU"),
        help="Hardware label used in latency caption")
