#!/usr/bin/env python3
"""
High-score stage-by-stage VQA trace report.

This script:
  1. Runs the full pipeline on selected/scanned test samples.
  2. Keeps only samples above a score threshold.
  3. Saves CSV, TXT, Markdown report with original images.
  4. Optionally generates Stage 7 Grad-CAM heatmaps.
  5. Generates a LaTeX .tex report including:
       - original image
       - Grad-CAM heatmap when available
       - Stage 1 to Stage 7 details
       - question, prediction, explanation, ground truth, metrics

Recommended commands:

  Small safe test without Grad-CAM:
    python show_stage_trace.py --scan_all --max_scan 50 --min_score 90 --score_mode avg --show_mem

  Small safe test with Grad-CAM:
    python show_stage_trace.py --scan_all --max_scan 20 --min_score 80 --score_mode avg --with_gradcam --show_mem

  500-sample scan with Grad-CAM for kept high-score samples:
    python show_stage_trace.py --scan_all --max_scan 500 --min_score 90 --score_mode avg --with_gradcam --show_mem

Compile LaTeX:
  cd ~/vqa_gi_thesis/logs/stage_trace
  pdflatex high_score_samples_report.tex
  pdflatex high_score_samples_report.tex
"""
...
def get_stage1_details(res):
    ...
    try:
        if disease_vec is not None:
            arr = np.array(disease_vec).reshape(-1)
            if arr.size > 0:
                conf = float(np.max(arr)) * 100.0
                return f"{disease_name} ({conf:.2f}% confidence)"
    except Exception:
        pass
...
def copy_sample_image(img_path, kept_no, img_id):
    try:
        ext = os.path.splitext(img_path)[1]
        safe_id = str(img_id).replace("/", "_").replace("\\", "_")
        saved_img_name = f"sample_{kept_no:04d}_{safe_id}{ext}"
        saved_img_path = os.path.join(IMG_OUT_DIR, saved_img_name)
...
def write_latex_report(df, args, processed, kept, latex_path):
...
\title{High-Score Stage-by-Stage Medical VQA Report}
\author{}
\date{}
...
This report shows high-score samples selected from the test set. Each example includes the original image, the question, stage-wise outputs, evaluation metrics, and the Stage 7 Grad-CAM visual explanation when available.

\begin{itemize}
""")
        f.write(r"    \item Threshold: " + latex_escape(args.min_score) + r"\%" + "\n")
        f.write(r"    \item Score mode: \texttt{" + latex_escape(args.score_mode) + r"}" + "\n")
        f.write(r"    \item Processed candidates: " + latex_escape(processed) + "\n")
        f.write(r"    \item Kept samples: " + latex_escape(kept) + "\n")
        f.write(r"\end{itemize}" + "\n\n")

        f.write(r"""\section*{Stage Meaning}

\begin{longtable}{p{0.20\textwidth} p{0.72\textwidth}}
\toprule
\textbf{Stage} & \textbf{Meaning} \\
\midrule
Stage 1 & Disease/finding classification from the endoscopic image. \\
Stage 2 & Question routing into the answer category. \\
Stage 3 & Multimodal fusion of visual, textual, and disease-context information. \\
Stage 4 & Structured short answer prediction. \\
Stage 5 & Verbalised answer sentence generated from the structured answer. \\
Stage 6 & Medical response: answer plus textual explanation and NLG metrics. \\
Stage 7 & Visual explanation using Grad-CAM heatmap. \\
\bottomrule
\end{longtable}
...
""")
        ...
        for _, r in df.iterrows():
            f.write(r"\clearpage" + "\n")
            f.write(r"\section*{Sample " + latex_escape(r["kept_sample"]) + r"}" + "\n\n")

            original_img = f"selected_images/{r['image_file']}"
            gradcam_img = str(r.get("stage7_heatmap", ""))

            f.write(r"\begin{figure}[H]" + "\n")
            f.write(r"\centering" + "\n")

            has_original = bool(r.get("image_file", ""))
            has_gradcam = gradcam_img.endswith(".png")

            if has_original and has_gradcam:
                f.write(r"\begin{subfigure}{0.47\textwidth}" + "\n")
                f.write(r"\centering" + "\n")
                f.write(r"\includegraphics[width=\linewidth]{" + original_img + r"}" + "\n")
                f.write(r"\caption{Original image}" + "\n")
                f.write(r"\end{subfigure}" + "\n")
                f.write(r"\hfill" + "\n")
                f.write(r"\begin{subfigure}{0.47\textwidth}" + "\n")
                f.write(r"\centering" + "\n")
                f.write(r"\includegraphics[width=\linewidth]{" + gradcam_img + r"}" + "\n")
                f.write(r"\caption{Stage 7 Grad-CAM}" + "\n")
                f.write(r"\end{subfigure}" + "\n")
            elif has_original:
                f.write(r"\includegraphics[width=0.58\textwidth]{" + original_img + r"}" + "\n")
            elif has_gradcam:
                f.write(r"\includegraphics[width=0.58\textwidth]{" + gradcam_img + r"}" + "\n")

            f.write(r"\caption{Original image and visual explanation when available.}" + "\n")
            f.write(r"\end{figure}" + "\n\n")

            f.write(r"\begin{longtable}{p{0.25\textwidth} p{0.68\textwidth}}" + "\n")
            f.write(r"\toprule" + "\n")
            f.write(r"\textbf{Component} & \textbf{Output} \\" + "\n")
            f.write(r"\midrule" + "\n")

            f.write(r"Image ID & " + latex_escape(r["img_id"]) + r" \\" + "\n")
            f.write(r"Question & " + latex_escape(r["question"]) + r" \\" + "\n")
            f.write(r"Stage 1 Disease & " + latex_escape(r["stage1_details"]) + r" \\" + "\n")
            f.write(r"Stage 2 Route & " + latex_escape(f"Route {r['route']} ({r['route_name']})") + r" \\" + "\n")
            f.write(r"Stage 3 Fusion & " + latex_escape(r["stage3_summary"]) + r" \\" + "\n")
            f.write(r"Stage 4 Structured Answer & " + latex_escape(r["stage4_answer"]) + r" \\" + "\n")
            f.write(r"Stage 5 Verbal Sentence & " + latex_escape(r["stage5_sentence"]) + r" \\" + "\n")
            f.write(r"Stage 6 Textual Explanation & " + latex_escape(r["stage6_explanation"]) + r" \\" + "\n")
            f.write(r"Ground Truth & " + latex_escape(r["ground_truth"]) + r" \\" + "\n")

            if has_gradcam:
                f.write(r"Stage 7 Visual Explanation & Grad-CAM heatmap generated and shown above. \\" + "\n")
            else:
                f.write(r"Stage 7 Visual Explanation & " + latex_escape(gradcam_img if gradcam_img else "Not generated") + r" \\" + "\n")
            ...
            f.write(r"\toprule" + "\n")
            f.write(r"Metric & Score \\" + "\n")
            f.write(r"\midrule" + "\n")
            f.write(r"ROUGE-1 & " + latex_escape(r["rouge1"]) + r" \\" + "\n")
            f.write(r"METEOR & " + latex_escape(r["meteor"]) + r" \\" + "\n")
            f.write(r"CHRF++ & " + latex_escape(r["chrf"]) + r" \\" + "\n")
            f.write(r"Average & " + latex_escape(r["avg_score"]) + r" \\" + "\n")
            f.write(r"\bottomrule" + "\n")
            f.write(r"\end{tabular}" + "\n")
            f.write(r"\caption{Stage 6 NLG evaluation metrics.}" + "\n")
            f.write(r"\end{table}" + "\n\n")
...
def main():
    ...
    ap.add_argument(
        "--no_latex",
        action="store_true",
        help="skip writing LaTeX report",
    )
    ...
    print(f"  gradcam    : {args.with_gradcam}")
    print("=" * 74 + "\n")
    ...
    for i, item in enumerate(selected, 1):
        try:
            ...
            # Pipeline prediction does not need gradients.
            ...
            disease_name = get_disease_name(res)
            stage1_details = get_stage1_details(res)
            stage3_summary = "Cross-attention fusion combines visual features, question features, and disease-context information."
            ...
            kept += 1
            saved_img_name = copy_sample_image(img_path, kept, img_id)

            heatmap_file = ""
            if gradcam is not None and transform is not None and s7 is not None:
                try:
                    # Grad-CAM needs gradients, so we explicitly enable gradients only here.
                    if torch is not None:
                    ...
                    heatmap_file = os.path.join(
                        OUT_DIR,
                        f"stage_trace_gradcam_{kept:04d}.png",
                    )
                    title = (
            ...
            stage7_value = (
                os.path.basename(heatmap_file)
                if heatmap_file and not heatmap_file.startswith("gradcam failed")
                else heatmap_file
            )
    ...
    latex_path = os.path.join(OUT_DIR, "high_score_samples_report.tex")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        for _, r in df.iterrows():
            ...
            f.write(f"Image file: selected_images/{r['image_file']}\n")
            f.write(f"Route: {r['route']} ({r['route_name']})\n")
            f.write(f"Disease: {r['disease_name']}\n")
            f.write(f"Stage 1: {r['stage1_details']}\n")
            f.write(f"Stage 3: {r['stage3_summary']}\n")
            ...
            f.write(f"Grad-CAM: {r['stage7_heatmap']}\n")
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Stage Meaning\n\n")
        f.write("| Stage | Meaning |\n")
        f.write("|---|---|\n")
        f.write("| Stage 1 | Disease/finding classification from image |\n")
        f.write("| Stage 2 | Question routing into answer category |\n")
        f.write("| Stage 3 | Multimodal visual-textual-disease fusion |\n")
        f.write("| Stage 4 | Structured short answer |\n")
        f.write("| Stage 5 | Verbalised answer sentence |\n")
        f.write("| Stage 6 | Medical explanation and NLG metrics |\n")
        f.write("| Stage 7 | Grad-CAM visual explanation |\n\n")
        ...
        for _, r in df.iterrows():
            ...
            if r.get("image_file", ""):
                f.write(f"![Sample image](selected_images/{r['image_file']})\n\n")
            ...
            f.write("|---|---|\n")
            f.write(f"| Stage 1 disease/finding | {r['stage1_details']} |\n")
            f.write(f"| Stage 2 route | Route {r['route']} ({r['route_name']}) |\n")
            f.write(f"| Stage 3 fusion | {r['stage3_summary']} |\n")
            ...
            f.write(f"| Stage 7 Grad-CAM | {r['stage7_heatmap']} |\n")
            f.write(f"| Ground truth | {r['ground_truth']} |\n\n")
    ...
    if not args.no_latex:
        write_latex_report(df, args, processed, kept, latex_path)
    ...
    if not args.no_latex:
        ...
        print(f"  Compile PDF          : cd {OUT_DIR} && pdflatex high_score_samples_report.tex && pdflatex high_score_samples_report.tex")
    print(f"  Copied images folder : {IMG_OUT_DIR}")
    if args.with_gradcam:
        print(f"  Grad-CAM heatmaps    : {OUT_DIR}/stage_trace_gradcam_*.png")
    print("=" * 74 + "\n")
